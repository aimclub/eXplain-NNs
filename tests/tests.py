...
def _test_bayes_prediction(use_wrapper: bool, mode: str):
    ...
    n_iter = 10
    if use_wrapper:
        res = bayes_api.BasicBayesianWrapper(
            model, **(params[mode])).predict(data, n_iter=n_iter)
    else:
        res = bayes_api.BasicBayesianPrediction(
            data, model, n_iter=n_iter, **(params[mode]))
...
def test_homologies():
    ...
    for layer, plot in res.items():
        utils.compare_values(matplotlib.figure.Figure, type(plot),
