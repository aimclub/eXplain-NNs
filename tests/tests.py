...
def test_visualization():
    ...
    utils.compare_values(set(['input'] + layers), set(res.keys()), 'Wrong dictionary keys')
    for key, plot in res.items():
        utils.compare_values(plotly.graph_objs._figure.Figure, type(plot), f'Wrong value type for key {key}')
...
def test_homologies():
    ...
    for layer, plot in res.items():
        utils.compare_values(matplotlib.figure.Figure, type(plot), f'Wrong result type for key {layer}')